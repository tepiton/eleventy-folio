# Memory: eleventy-folio

## Project Overview

This is the `eleventy-folio` template repo (`github.com/tepiton/eleventy-folio`), an Eleventy v3 starter for chaptered literary sites with polished defaults. It is one of three interoperable templates:
...
content/
  ...
  chapters/
    ...
    *.md                  # order + title in frontmatter (optional dek)
...
- Has dedicated `home.njk` layout
- Chapter frontmatter supports optional `dek` (subtitle)
- Uses `@zachleat/heading-anchors` for anchor links
- Includes sitemap generation via `sitemap.xml.njk`
